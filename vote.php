...
if(isset($_POST['submit_vote'])){
    ...
    while($row_positions = mysqli_fetch_assoc($result_positions)){
        ...
        $sql_update = "UPDATE candidates SET num_of_votes = num_of_votes + 1 WHERE CandidateID = '$candidate_id'";
...
// If the form is not submitted, display the list of positions and candidates
$sql_positions = "SELECT * FROM positions";
$result_positions = mysqli_query($conn, $sql_positions);
...
while($row_positions = mysqli_fetch_assoc($result_positions)){
    ...
    $sql_candidates = "SELECT * FROM candidates WHERE position_id = '$position_id'";
